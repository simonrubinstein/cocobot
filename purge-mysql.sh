...
mysql -u test -pGim9p6gW test -e "DELETE FROM users where id_code IN (SELECT ID FROM codes WHERE code IN ('kVO', '6eF'));" 
...
mysql -u test -pGim9p6gW test -e "DELETE FROM nicknames WHERE id NOT IN (SELECT distinct id_mynickname from users);"

mysql -u test -pGim9p6gW test -e "SELECT COUNT(*) FROM codes; SELECT COUNT(*) FROM users; SELECT COUNT(*) FROM nicknames;"
